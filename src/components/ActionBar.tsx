...
export default function ActionBar({ room, playerId, onAction, loading }: ActionBarProps) {
  ...
  const nextBet = Math.min(room.currentBet * 2, maxBet);
  const raiseBet = player.hasSeen ? nextBet * 2 : nextBet;
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 glass-bar safe-bottom z-40">
      <div className="max-w-lg mx-auto px-4 py-3">
        {/* Turn indicator */}
        <p className="text-accent text-xs text-center mb-2.5 font-medium tracking-wide">Your turn</p>
        ...
        <div className="flex gap-2 justify-center flex-wrap">
          ...
          {validActions.includes('blind') && (
            <button
              ...
              disabled={loading || player.chips < blindBet}
              className="px-4 py-2 bg-surface border border-border rounded text-sm text-primary
                transition-colors hover:border-muted active:scale-95 disabled:opacity-40"
            >
              <span>Blind</span>
          ...
          {validActions.includes('chaal') && (
            <button
              ...
              disabled={loading || player.chips < seenBet}
              className="px-4 py-2 bg-surface border border-border rounded text-sm text-primary
                transition-colors hover:border-muted active:scale-95 disabled:opacity-40"
            >
              <span>Chaal</span>
          ...
          {validActions.includes('raise') && (
            <button
              ...
              disabled={loading || player.chips < raiseBet}
          ...
          {validActions.includes('show') && (
            <button
              ...
              disabled={loading || player.chips < showCost}
              className="px-4 py-2 bg-accent text-white rounded text-sm font-medium
                transition-colors hover:bg-accent-dim active:scale-95 disabled:opacity-40 flex items-center"
            >
              <span>Show cards</span>
              <span className="text-white/80 text-xs ml-1.5 font-normal">({showCost})</span>
            </button>
          )}
